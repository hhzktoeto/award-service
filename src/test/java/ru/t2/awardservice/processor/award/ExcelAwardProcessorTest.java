...
class ExcelAwardProcessorTest {

    @Mock
    private EmployeeService mockEmployeeService;
    @InjectMocks
    private ExcelAwardProcessor processor;

    private final List<Award> testAwards = TestData.awards();

    private final MultipartFile validAwardsXlsFile = TestData.validAwardsXLS();
    private final MultipartFile validAwardsXlsxFile = TestData.validAwardsXLSX();
    private final MultipartFile invalidAwardsXlsxFile = TestData.invalidAwardsXLSX();

    @Test
    void process_validXlsFile_shouldReturnParsedAwards() {
        when(mockEmployeeService.exists(anyLong())).thenReturn(true);

        List<Award> result = processor.process(validAwardsXlsFile);
    ...
    @Test
    void process_validXlsxFile_shouldReturnParsedAwards() {
        when(mockEmployeeService.exists(anyLong())).thenReturn(true);

        List<Award> result = processor.process(validAwardsXlsxFile);
    ...
    void process_fileIsInvalid_shouldThrowIncorrectFileDataException() {
        assertAll(
                () -> assertThrows(IncorrectFileDataException.class, () -> processor.process(invalidAwardsXlsxFile)),
